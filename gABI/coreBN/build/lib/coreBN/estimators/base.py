...
class BaseEstimator(object):
    def __init__(self, data=None, state_names=None, complete_samples_only=True):
        """
        Base class for estimators in core; `ParameterEstimator`,
        ...
        state_names: dict (optional, and used only for discrete vars)
            A dict indicating, for each variable, the discrete set of states (or values)
            that the variable can take. 
            If unspecified, the observed values in the data set are taken to be the only possible states.

        complete_samples_only: bool (optional, default `True`)
            Specifies how to deal with missing data, if present. 
            If set to `True` all rows that contain `np.Nan` somewhere are ignored. 
            If `False` then, for each variable, every row where neither the variable nor its parents are `np.NaN` is used.
...
class ParameterEstimator(BaseEstimator):
    def __init__(self, model, data, **kwargs):
        ...
        Parameters
        ----------
        model: coreBN.models.BayesianNetwork or coreBN.models.MarkovNetwork 
...
class StructureEstimator(BaseEstimator):
    def __init__(self, data=None, independencies=None, **kwargs):
        """
        Base class for structure estimators
