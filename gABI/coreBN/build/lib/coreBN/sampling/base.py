...
class BaseGradLogPDF(object):
    ...
    model : An instance of coreBN.models

    Examples
    --------
    >>> from coreBN.factors import GaussianDistribution
    >>> from coreBN.inference.continuous import BaseGradLogPDF
    >>> import numpy as np
    ...
    def get_gradient_log_pdf(self):
        ...
        >>> from coreBN.sampling.base import GradLogPDFGaussian
        >>> from coreBN.factors.continuous import GaussianDistribution
        >>> import numpy as np
        >>> mean = np.array([1, 1])
...
class GradLogPDFGaussian(BaseGradLogPDF):
    ...
    model : An instance of coreBN.models.GaussianDistribution

    Example
    -------
    >>> from coreBN.sampling import GradLogPDFGaussian
    >>> from coreBN.factors.continuous import GaussianDistribution
    >>> import numpy as np
    >>> mean = np.array([3, 4])
...
class BaseSimulateHamiltonianDynamics(object):
    ...
    grad_log_pdf : A subclass of coreBN.inference.continuous.BaseGradLogPDF
    ...
    Examples
    --------
    >>> from coreBN.sampling import BaseSimulateHamiltonianDynamics
    >>> from coreBN.factors.continuous import GaussianDistribution
    >>> from coreBN.sampling import GradLogPDFGaussian
    >>> import numpy as np
    ...
    def __init__(
        ...
        if not issubclass(grad_log_pdf, BaseGradLogPDF):
            raise TypeError(
                ...
                + " of coreBN.inference.continuous.base.BaseGradLogPDF"
            )
    ...
    def get_proposed_values(self):
        ...
        >>> from coreBN.inference.continuous import ModifiedEuler, GradLogPDFGaussian as GLPG
        >>> from coreBN.factors import GaussianDistribution
        >>> import numpy as np
        >>> pos = np.array([3, 4])
...
class LeapFrog(BaseSimulateHamiltonianDynamics):
    ...
    Inherits coreBN.inference.base_continuous.BaseSimulateHamiltonianDynamics
    ...
    --------
    >>> from coreBN.factors.continuous import GaussianDistribution
    >>> from coreBN.sampling import LeapFrog, GradLogPDFGaussian as GLPG
    >>> import numpy as np
    >>> pos = np.array([2, 1])
...
class ModifiedEuler(BaseSimulateHamiltonianDynamics):
    ...
    --------
    >>> from coreBN.factors.continuous import GaussianDistribution
    >>> from coreBN.sampling import GradLogPDFGaussian, ModifiedEuler
    >>> import numpy as np
    >>> pos = np.array([2, 1])
    >>> momentum = np.array([1, 1])
    >>> mean = np.array([0, 0])
