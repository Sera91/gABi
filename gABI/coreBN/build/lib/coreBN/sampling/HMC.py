# -*- coding: UTF-8 -*-
"""
    A collection of methods for sampling from continuous models in coreBN
"""
...
from coreBN.sampling import (
    BaseGradLogPDF,
    BaseSimulateHamiltonianDynamics,
    LeapFrog,
    _return_samples,
)
from coreBN.utils import _check_1d_array_object, _check_length_equal
...
class HamiltonianMC(object):
    ...
    Parameters
    ----------
    model: An instance coreBN.models
        Model from which sampling has to be done

    grad_log_pdf: A subclass of coreBN.inference.continuous.BaseGradLogPDF, defaults to None
    ...
    simulate_dynamics: A subclass of coreBN.inference.continuous.BaseSimulateHamiltonianDynamics
        A class to propose future values of momentum and position in time by simulating
        Hamiltonian Dynamics

    Example
    -------
    >>> from coreBN.sampling import HamiltonianMC as HMC, LeapFrog, GradLogPDFGaussian
    >>> from coreBN.factors.continuous import GaussianDistribution as JGD
    >>> import numpy as np
    >>> mean = np.array([-3, 4])
    ...
    def __init__(self, model, grad_log_pdf, simulate_dynamics=LeapFrog):
        ...
        if not issubclass(grad_log_pdf, BaseGradLogPDF):
            raise TypeError(
                ...
                + "coreBN.inference.base_continuous.BaseGradLogPDF"
            )

        if not issubclass(simulate_dynamics, BaseSimulateHamiltonianDynamics):
            raise TypeError(
                "split_time must be an instance of "
                + "coreBN.inference.base_continuous.BaseSimulateHamiltonianDynamics"
            )
    ...
    def sample(
        ...
        Examples
        --------
        >>> from coreBN.sampling import HamiltonianMC as HMC, GradLogPDFGaussian, ModifiedEuler
        >>> from coreBN.factors.continuous import GaussianDistribution as JGD
        >>> import numpy as np
        >>> mean = np.array([1, -1])
    ...
    def generate_sample(
        ...
        Examples
        --------
        >>> from coreBN.sampling import HamiltonianMC as HMC, GradLogPDFGaussian as GLPG
        >>> from coreBN.factors import GaussianDistribution as JGD
        >>> import numpy as np
        >>> mean = np.array([4, -1])
...
class HamiltonianMCDA(HamiltonianMC):
    ...
    grad_log_pdf: A subclass of coreBN.inference.continuous.GradientLogPDF
    ...
    Example
    -------
    >>> from coreBN.sampling import HamiltonianMCDA as HMCda, LeapFrog, GradLogPDFGaussian as GLPG
    >>> from coreBN.factors.continuous import GaussianDistribution as JGD
    >>> import numpy as np
    >>> mean = np.array([1, 2, 3])
    ...
    def sample(
        ...
        Examples
        ---------
        >>> from coreBN.sampling import HamiltonianMCDA as HMCda, GradLogPDFGaussian as GLPG, LeapFrog
        >>> from coreBN.factors.continuous import GaussianDistribution as JGD
        >>> import numpy as np
        >>> mean = np.array([1, 1])
