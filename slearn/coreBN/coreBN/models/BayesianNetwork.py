...
from coreBN.factors.discrete import (
    DiscreteFactor,
...
class BayesianNetwork(DAG):
    ...
    def fit(
        ...
        **kwargs,
        ...
        """
        from coreBN.estimators import BaseEstimator, MaximumLikelihoodEstimator

        if estimator is None:
            estimator = MaximumLikelihoodEstimator
        else:
            if not issubclass(estimator, BaseEstimator):
                raise TypeError("Estimator object should be a valid slearn estimator.")
    ...
    def fit_update(self, data, n_prev_samples=None, n_jobs=-1):
        ...
        >>> from coreBN.sampling import BayesianModelSampling
    ...
    def copy(self):
        ...
        Returns
        -------
        Model's copy: coreBN.models.BayesianNetwork
    ...
    def do(self, nodes, inplace=False):
        ...
        Returns
        -------
        Modified network: coreBN.models.BayesianNetwork or None
    ...
    def simulate(
        ...
        coreBN.sampling.BayesianModelSampling to generate the data.
        ...
        virtual_evidence: list
            ...
            be a list of `core.factors.discrete.TabularCPD` objects specifying the
            virtual probabilities.

        virtual_intervention: list
            Also known as soft intervention. `virtual_intervention` should be a list
            of `core.factors.discrete.TabularCPD` objects specifying the virtual/soft
    ...
    def load(filename, filetype="bif"):
        ...
        if filetype == "bif":
            ...
            from coreBN.IO import UAIReader

            reader = UAIReader(path=filename)
            return reader.get_model()

        elif filetype == "xmlbif":
            from coreBN.IO import XMLBIFReader
