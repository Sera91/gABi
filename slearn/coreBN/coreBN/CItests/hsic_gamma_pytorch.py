...
def Hsic_gamma_py(x, y, n_device, sigma=1.0, debug=False, gpu_selected=True):
    ...
    if go_gpu:
        ...
        with dev.use():
            gpu_x_arr = cu.asarray(x, dtype='float64')
            gpu_y_arr = cu.asarray(y, dtype='float64')
            threadsperblock = (16, 16)
            blockspergrid_x = ceil(N_obs / threadsperblock[0])
            blockspergrid_y = ceil(N_obs / threadsperblock[1])
            blockspergrid = (blockspergrid_x, blockspergrid_y)
            K = cu.ones((N_obs, N_obs), dtype='float64')
            GPU_kernel_matrix[blockspergrid, threadsperblock](gpu_x_arr,N_obs,K, 1.0)
            L = cu.ones((N_obs, N_obs), dtype='float64')
            GPU_kernel_matrix[blockspergrid, threadsperblock](gpu_y_arr,N_obs,L, 1.0)
        ...
        kernel_K = CPU_kernel_matrix(x,N_obs, sigma=1.0)
        tensor_K = torch.from_numpy(kernel_K.astype(string_Ttype)).to(tensor_device)
        kernel_L = CPU_kernel_matrix(y,N_obs, sigma=1.0)
    ...
    factor= 1./N_obs/(N_obs-1)
    mu_K = (factor*torch.sum(tensor_K - torch.diag(tensor_K)*torch.eye(N_obs, device=tensor_device))).to('cpu').numpy()
    mu_L = (factor*torch.sum(tensor_L - torch.diag(tensor_L)*torch.eye(N_obs, device=tensor_device))).to('cpu').numpy()

    #HSIC_testStat = torch.sum(torch.multiply(torch.t(Kc), Lc))/N_obs
    #if debug:
    #    print("hsic", HSIC_testStat)
    HSIC_testStat = (torch.sum(torch.multiply(torch.t(Kc), Lc))).to('cpu').numpy()/(N_obs*N_obs)
    ...
    varHSIC = torch.pow((1/6.0 * torch.multiply(Kc, Lc)),2.0)
    if debug:
        print("varHSIC:", varHSIC)
  
    Var_HSIC = 1/N_obs/(N_obs-1)* ( torch.sum(varHSIC) - torch.trace(varHSIC)) 
    Var_HSIC = ((72*(N_obs-4)*(N_obs-5)/N_obs/(N_obs-1)/(N_obs-2)/(N_obs-3))  *  Var_HSIC).to('cpu').numpy()  #variance under H0
