...
from coreBN.CItests.CITests import chi_square, independence_match, pearsonr, Fisher_Z_test, CItest_cycle
...
class PC(StructureEstimator):
    ...
    def estimate(
        ...
        **kwargs,
        ...
        skel, separating_sets = self.build_skeleton( ci_test=ci_test, max_cond_vars=max_cond_vars, client=iclient, significance_level=significance_level,
    ...
    def build_skeleton(
        ...
        **kwargs,
        ...
        if (variant == "dask-parallel"):
            ...
            corr_matrix = corr_matrix(data_matrix)
            client.scatter(corr_matrix)
            separating_sets = dict()

            def parallel_CItest(u, v, sep_sets,l_m):
                    if l_m ==0:
                        #coef, p_value = stats.pearsonr(data[u].to_numpy(), data[v].to_numpy())
                        if ci_test(u,v,([]), data=self.data,
                            significance_level=significance_level,**kwargs,):
                                return np.array([(u, v), ([])])                        
                        return np.array([None, None])
                    
                    list_sets =list(sep_sets)
                    for separating_set in list_sets:
                        if ci_test(u,v,separating_set, data=self.data,
                            significance_level=significance_level,**kwargs,):
                                return np.array([(u, v), separating_set])
            ...
            ) and (lim_neighbors <= max_cond_vars):
