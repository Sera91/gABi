...
import pypickle
import slearn
import slearn.Netk as sl_network

from coreBN.models import BayesianNetwork
from coreBN.factors.discrete import TabularCPD
from coreBN.sampling import BayesianModelSampling  # GibbsSampling
from coreBN import IO as readwrite

from ismember import ismember


import slearn.data_utils as dutils
...
def to_bayesiannetwork(model, verbose=3):
    ...
    if isinstance(model, dict):
        ...
        adjmat = model
    if adjmat is None: raise Exception('[slearn] >Error: input for "bayesiannetwork" should be adjmat or a dict containing a key "adjmat".')

    if verbose>=3: print('[slearn] >Conversion of adjmat to BayesianNetwork.')
...
def make_DAG(DAG, CPD=None, methodtype='bayes', checkmodel=True, verbose=3):
    ...
    >>> import slearn as bn
    >>> edges = [('A', 'B'), ('A', 'C'), ('A', 'D')]
    >>> DAG = sl.make_DAG(edges, methodtype='bayes')
    >>> sl.plot(DAG)
    ...
    if (not isinstance(DAG, list)) and ('core' not in str(type(DAG))):
        raise Exception("[slearn] >Error: Input DAG should be a list. in the form [('A','B'), ('B','C')] or a <coreBN.models.BayesianNetwork>")
    elif ('core' in str(type(DAG))):
        # Extract methodtype from existing model.
        if ('bayesianmodel' in str(type(DAG)).lower()):
            methodtype='bayes'
        
        if verbose>=3: print('[slearn] >No changes made to existing %s DAG.' %(methodtype))
    
    elif isinstance(DAG, list) and methodtype=='bayes':
        if verbose>=3: print('[slearn] >%s DAG created.' %(methodtype))
        DAG = BayesianNetwork(DAG)

    if CPD is not None:
        for cpd in CPD:
            DAG.add_cpds(cpd)
            if verbose>=3: print('[slearn] >Add CPD: %s' %(cpd.variable))
        # Check model
        if checkmodel:
...
def print_CPD(DAG, checkmodel=False):
    ...
    Parameters
    ----------
    DAG : coreBN.models.BayesianNetwork
        model of the DAG.
    checkmodel : bool
        Check the validity of the model. The default is True
    ...
    try:
        ...
        elif ('bayesiannetwork' in str(type(DAG)).lower()):
            # print CPDs using Bayesian Parameter Estimation
            if len(DAG.get_cpds())==0:
                raise Exception('[slearn] >Error! This is a Bayesian DAG containing only edges, and no CPDs. Tip: you need to specify or learn the CPDs. Try: DAG=sl.parameter_learning.fit(DAG, df). At this point you can make a plot with: sl.plot(DAG).')
                return
            for cpd in DAG.get_cpds():
                print("CPD of {variable}:".format(variable=cpd.variable))
                print(cpd)
            if ('bayesiannetwork' in str(type(DAG)).lower()):
                print('[slearn] >Independencies:\n%s' %(DAG.get_independencies()))
            print('[slearn] >Nodes: %s' %(DAG.nodes()))
            print('[slearn] >Edges: %s' %(DAG.edges()))

        if checkmodel:
            _check_model(DAG, verbose=3)
    except:
        print('[slearn] >No CPDs to print. Hint: Add CPDs as following: <sl.make_DAG(DAG, CPD=[cpd_A, cpd_B, etc])> and use slearn.plot(DAG) to make a plot.')


# %%
def _check_model(DAG, verbose=3):
    if verbose>=3: print('[slearn] >Checking CPDs..')
    for cpd in DAG.get_cpds():
        # print(cpd)
        if not np.all(cpd.values.sum(axis=0)==1):
            print('[slearn] >Warning: CPD [%s] does not add up to 1 but is: %s' %(cpd.variable, cpd.values.sum(axis=0)))
    if verbose>=3:
        print('[slearn] >Check for DAG structure. Correct: %s' %(DAG.check_model()))
...
def dag2adjmat(model, verbose=3):
    ...
    >>> import slearn as bn
    >>> # Load DAG
    >>> DAG = sl.import_DAG('Sprinkler')
    >>> # Extract edges from model and store in adjacency matrix
    >>> adjmat=sl.dag2adjmat(DAG['model'])
    ...
    if hasattr(model, 'nodes') and hasattr(model, 'edges'):
        ...
        if verbose>=1: print('[slearn] >Could not convert to adjmat because nodes and/or edges were missing.')
    return(adjmat)
...
def vec2df(source, target, weights=None):
    ...
    >>> weights=[1,2,1,3]
    >>> df = sl.vec2df(source, target, weights=weights)

    >>> Example 2
    >>> import slearn as bn
    >>> vec = sl.import_example("stormofswords")
    >>> df = sl.vec2df(vec['source'], vec['target'], weights=vec['weight'])
...
def vec2adjmat(source, target, weights=None, symmetric=True):
    ...
    >>> weights=[1,2,1,3]
    >>> adjmat = sl.vec2adjmat(source, target, weights=weights)
...
def sampling(DAG, n=1000, verbose=3):
    ...
    if verbose>=3: print('[slearn] >Forward sampling for %.0d samples..' %(n))
...
def _bif2bayesian(pathname, verbose=3):
    ...
    >>> reader.get_model()
    <coreBN.models.BayesianNetwork object at 0x7f20af154320>
    """
    if verbose>=3: print('[slearn] >Loading bif file <%s>' %(pathname))
...
#                model.node[node][prop_name] = prop_value
        ...
        raise AttributeError('[slearn] >First get states of variables, edges, parents and network names')
...
def compare_networks(model_1, model_2, pos=None, showfig=True, figsize=(15, 8), verbose=3):
    ...
    """
    scores, adjmat_diff = slearn.network.compare_networks(model_1['adjmat'], model_2['adjmat'], pos=pos, showfig=showfig, width=figsize[0], height=figsize[1], verbose=verbose)
...
def get_node_properties(model, node_color='#ffffff', node_size=None, verbose=3):
    ...
    >>> model = sl.make_DAG(edges)
    >>> node_properties = sl.get_node_properties(model)
    >>> # Adjust the properties
    >>> node_properties['A']['node_size']=2000
    >>> node_properties['A']['node_color']='#000000'
    >>> # Make plot
    >>> sl.plot(model, interactive=False, node_properties=node_properties)
    >>>
    >>> # Example: Specify all nodes
    >>> node_properties = sl.get_node_properties(model, node_size=1000, node_color='#000000')
    >>> sl.plot(model, interactive=False, node_properties=node_properties)
    ...
    if adjmat is not None:
        if verbose>=3: print('[slearn] >Set node properties.')
...
def get_edge_properties(model, color='#000000', weight=1, minscale=1, maxscale=10, verbose=3):
    ...
    >>> # Create DAG and store in model
    >>> model = sl.make_DAG(edges)
    >>> edge_properties = sl.get_edge_properties(model)
    >>> # Adjust the properties
    >>> edge_properties[('A', 'B')]['weight']=10
    >>> edge_properties[('A', 'B')]['color']='#8A0707'
    >>> # Make plot
    >>> sl.plot(model, interactive=False, edge_properties=edge_properties)
    >>>
    >>> # Example 2:
    >>>  # Load asia DAG
    >>> df = sl.import_example(data='asia')
    >>> # Structure learning of sampled dataset
    >>> model = sl.structure_learning.fit(df)
    >>> # Compute edge weights based on chi_square test statistic
    >>> model = sl.independence_test(model, df, test='chi_square')
    >>> # Get the edge properties
    >>> edge_properties = sl.get_edge_properties(model)
    >>> # Make adjustments
    >>> edge_properties[('tub', 'either')]['color']='#8A0707'
    >>> # Make plot
    >>> sl.plot(model, interactive=True, edge_properties=edge_properties)
    ...
    if adjmat is not None:
        if verbose>=3: print('[slearn]> Set edge weights based on the [%s] test statistic.' %(model['independence_test'].columns[-2]))
    ...
    if adjmat is not None:
        if verbose>=3: print('[slearn] >Set edge properties.')
...
def plot(model,
    ...
    node_properties : dict (default: None)
        ...
        The node properties can easily be retrieved using the function: node_properties = sl.get_node_properties(model)
    ...
    >>>
    >>> # plot interactive
    >>> G = sl.plot(model, interactive=True)
    >>>
    >>> # plot interactive with various settings
    >>> sl.plot(model, interactive=True, node_color='#8A0707', node_size=35, params_interactive = {'height':'800px', 'width':'70%', 'bgcolor':'#0f0f0f0f'})
    >>>
    ...
    >>> # Plot
    >>> sl.plot(model, interactive=True, node_properties=node_properties)
    >>>

    """
    # Check whether edges are available
    if model['adjmat'].sum().sum()==0:
        if verbose>=3: print('[slearn]> Nothing to plot because no edges are present between nodes. ')
        return None
    ...
    if (params_static.get('width') is not None) or (params_static.get('height') is not None):
        # if verbose>=2: print('[slearn]> Warning: [height] and [width] will be removed in further version. Please use: params_static={"figsize": (15, 10)}.')
    ...
    if (node_properties is not None) and (node_size is not None):
        if verbose>=2: print('[slearn]> Warning: if both "node_size" and "node_properties" are used, "node_size" will be used.')

    # Get node and edge properties if not user-defined
    if node_properties is None:
        node_properties = slearn.get_node_properties(model, node_size=node_size_default)
    if edge_properties is None:
        edge_properties = slearn.get_edge_properties(model, minscale=params_static['minscale'], maxscale=params_static['maxscale'])
    ...
    # Bayesian model
    if ('bayes' in str(type(bnmodel)).lower()) or ('core' in str(type(bnmodel)).lower()):
        if verbose>=3: print('[slearn] >Plot based on Bayesian model')
        # positions for all nodes
        G = nx.DiGraph(model['adjmat'])
        #pos = slearn.network.graphlayout(G, pos=pos, scale=scale, layout=params_static['layout'], verbose=verbose)
        pos = sl_network.graphlayout(G, pos=pos, scale=scale, layout=params_static['layout'], verbose=verbose)
    elif 'networkx' in str(type(bnmodel)):
        if verbose>=3: print('[slearn] >Plot based on networkx model')
        G = bnmodel
        #pos = slearn.network.graphlayout(G, pos=pos, scale=scale, layout=params_static['layout'], verbose=verbose)
        pos = sl_network.graphlayout(G, pos=pos, scale=scale, layout=params_static['layout'], verbose=verbose)
    else:
        if verbose>=3: print('[slearn] >Plot based on adjacency matrix')
        G = slearn.network.adjmat2graph(model['adjmat'])
        # Get positions
        #pos = slearn.network.graphlayout(G, pos=pos, scale=scale, layout=params_static['layout'], verbose=verbose)
        pos = sl_network.graphlayout(G, pos=pos, scale=scale, layout=params_static['layout'], verbose=verbose)
...
def topological_sort(adjmat, start=None):
    ...
    Parameters
    ----------
    adjmat: pd.DataFrame or slearn object.
        Adjacency matrix.
    start: str, optional
    ...
    -----------
    import slearn as sl
    DAG = sl.import_DAG('sprinkler', verbose=0)
    sl.topological_sort(DAG, 'Rain')
    sl.topological_sort(DAG)
...
def import_example(data='sprinkler', n=10000, verbose=3):
    ...
    if (data=='sprinkler') or (data=='titanic'):
        ...
        try:
            ...
            print('[slearn] >Error: Loading data not possible!')
            df = None
...
def import_DAG(filepath='sprinkler', CPD=True, checkmodel=True, verbose=3):
    ...
    if verbose>=3: print('[slearn] >Import <%s>' %(filepath))
    ...
    if filepath=='sprinkler':
        ...
        if not os.path.isfile(getfile):
            PATH_TO_DATA = dutils.download_example(filepath, verbose=3)
            _ = _unzip(PATH_TO_DATA, verbose=verbose)
        ...
        if os.path.isfile(filepath):
            ...
            if verbose>=3: print('[slearn] >filepath does not exist! <%s>' %(filepath))
            return(out)
...
def _unzip(getZip, ext='.bif', verbose=3):
    if not os.path.isdir(getZip):
        if verbose>=3: print('[slearn] >Extracting files..')
...
def _filter_df(adjmat, df, verbose=3):
    ...
    if len(remcols)>0:
        if verbose>=3: print('[slearn] >Removing columns from dataframe to make consistent with DAG [%s]' %(remcols))
...
def predict(model, df, variables, to_df=True, method='max', verbose=3):
    ...
    ----------
    model : Object
        An object of class from sl.fit.
    df : pd.DataFrame
    ...
    """
    if not isinstance(df, pd.DataFrame): raise Exception('[slearn] >Error: Input requires a pd.DataFrame.')
    if not isinstance(model, dict): raise Exception('[slearn] >Error: Input requires a dict that contains the key: model.')
    if isinstance(variables, str): variables=[variables]
    # Remove columns that are used as priors
    dfX = df.loc[:, ~np.isin(df.columns.values, variables)]
    if verbose>=3: print('[slearn]> Remaining columns for inference: %d' %(dfX.shape[1]))
    ...
    for evidence in tqdm(evidences):
        ...
        # Do the inference.
        query = slearn.inference.fit(model, variables=variables, evidence=evidence, to_df=False, verbose=0)
...
def _get_prob(query, method='max'):
    ...
    if method=='max':
        ...
        out = slearn.query2df(query).to_dict()
    return out


# %% Save model
def save(model, filepath='slearn_model.pkl', overwrite=False, verbose=3):
    """Save learned model in pickle file.

    Parameters
    ----------
    filepath : str, (default: 'slearn_model.pkl')
...
# %% Load model.
def load(filepath='slearn_model.pkl', verbose=3):
    """Load learned model.
    ...
    if model is not None:
        ...
        if verbose>=2: print('[slearn] >WARNING: Could not load data from [%s]' %(filepath))
...
def independence_test(model, df, test="chi_square", alpha=0.05, prune=False, verbose=3):
    ...
    Parameters
    ----------
    model: Instance of slearn.structure_learning.
    ...
    """
    from coreBN.estimators.CITests import chi_square, g_sq, log_likelihood, freeman_tuckey, modified_log_likelihood, neyman, cressie_read  # noqa
    from coreBN.models import BayesianNetwork
    from coreBN.base import DAG
    if model.get('model', None) is None: raise ValueError('[slearn]> No model detected.')
    if not isinstance(model['model'], (DAG, BayesianNetwork)): raise ValueError("[slearn]> model must be an instance of core.base.DAG or coreBN.models.BayesianNetwork. Got {type(model)}")
    if not isinstance(df, pd.DataFrame): raise ValueError("[slearn]> data must be a pandas.DataFrame instance. Got {type(data)}")
    # if set(model['model'].nodes()) != set(df.columns): raise ValueError("[slearn]> Missing columns in data. Can't find values for the following variables: { set(model.nodes()) - set(data.columns) }")
    if not np.all(np.isin(model['model'].nodes(), df.columns)): raise ValueError("[slearn]> Missing columns in data. Can't find values for the following variables: { set(model.nodes()) - set(data.columns) }")
    if verbose>=3: print('[slearn] >Compute edge strength with [%s]' %(test))
...
def _prune(model, test, alpha, verbose=3):
    ...
    if independence_test is not None:
        ...
        for idx in idxrem:
            ...
            # Remove from list
            if verbose>=3: print('[slearn] >Edge [%s <-> %s] [P=%g] is excluded because it was not significant (P<%.2f) with [%s]' %(edge[0], edge[1], model['independence_test'].iloc[idx]['p_value'], alpha, test))
